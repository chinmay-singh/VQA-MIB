...
class Net(nn.Module):
    ...
    def __init__(self, __C, pretrained_emb, token_size, answer_size):
        ...
        # Classification layers
        self.norm = LayerNorm(__C.FLAT_OUT_SIZE)
        self.to_flat_out_size = nn.Linear( __C.HIDDEN_SIZE, __C.FLAT_OUT_SIZE)
        self.to_answer_vocab_size = nn.Linear(2*__C.HIDDEN_SIZE, answer_size)
    ...
    def forward(self, frcn_feat, grid_feat, bbox_feat, ques_ix):
        ...
        # Language mask
        lang_feat_mask = make_mask(ques_ix.unsqueeze(2)) # (batch, 1, 1, 14)
        lang_feat_mask = lang_feat_mask.squeeze(1).squeeze(1).unsqueeze(2) # (batch, 14, 1)
        lang_feat_mask = lang_feat_mask.expand(lang_feat.shape) # (batch, 14, 512)

        # Applying language mask
        lang_feat = lang_feat.masked_fill(lang_feat_mask, 0)

        # Image Features and mask
        img_feat, img_feat_mask = self.adapter(frcn_feat, grid_feat, bbox_feat) # (batch, 100, 512), (batch, 1, 1, 100)

        # Applying image mask
        img_feat_mask = img_feat_mask.squeeze(1).squeeze(1).unsqueeze(2) # (batch, 14, 1)
        img_feat_mask = img_feat_mask.expand(img_feat.shape) # (batch, 14, 512)
        img_feat = img_feat.masked_fill(img_feat_mask, 0)

        # To flat out size
        img_feat = self.to_flat_out_size(img_feat)
        img_feat = img_feat.masked_fill(img_feat_mask, -1e-9)
        img_feat = self.to_flat_out_size(img_feat)

        # Classification layers
        proj_feat = lang_feat + img_feat
        proj_feat = self.norm(proj_feat)
        proj_feat = self.to_answer_vocab_size(proj_feat)
