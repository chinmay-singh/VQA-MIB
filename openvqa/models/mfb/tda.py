...
class CoAtt_vis(nn.Module):
    def __init__(self, __C):
        super(CoAtt_vis, self).__init__()
        self.__C = __C
        ...
        #################################################################
        #####Also getting the list of attention map of the glimpes####### 
        
        self.i_att_list = IAtt(__C, img_feat_size, ques_att_feat_size)

        # self.iatt = torch.cat(self.iatt_feat_list, dim=1)
    ...
    def forward(self, img_feat, ques_feat , bbox_feat):
        ...
        fused = self.i_att(img_feat, ques_feat)     # (N, FRCN_FEAT_SIZE*I_GLIMPSES)
        ...
        if self.__C.USE_NEW_QUESTION == "True":
            ...
            img2 = np.array(img).astype('float64')
            ...
            for i in self.i_att_list:
                att_squeezed.append(i.squeeze())
