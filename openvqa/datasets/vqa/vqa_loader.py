...
class DataSet(BaseDataSet):
    def __init__(self, __C):
        ...
        if os.path.exists('cache/vqa_' + __C.RUN_MODE + '.pkl'):
            ...
            print('Loading the dataset from cache...')

            # load from pickle file
            vqadataset = pickle.load(open('cache/vqa_' + __C.RUN_MODE + '.pkl', 'rb'))

            # save into variables
            self.ques_list = vqadataset['ques_list']
            self.ans_list = vqadataset['ans_list']
            self.iid_to_frcn_feat_path = vqadataset['iid_to_frcn_feat_path']
            self.qid_to_ques = vqadataset['qid_to_ques']
            self.token_to_ix = vqadataset['token_to_ix']
            self.pretrained_emb = vqadataset['pretrained_emb']
            self.token_size = vqadataset['token_size']
            self.ans_to_ix = vqadataset['ans_to_ix']
            self.ix_to_ans = vqadataset['ix_to_ans']
            self.ans_size = vqadataset['ans_stat']

            cache_end_time = time.time()

            print("Time in loading {} set from cache is {}s".format(__C.RUN_MODE, cache_end_time - cache_start_time))
            ...
            print("{} set loading time from scratch is {}s".format(__C.RUN_MODE, raw_end_time - raw_start_time))


            if __C.SAVE_TO_CACHE:
                # save into cache variables
                vqadataset = {}

                vqadataset['ques_list'] = self.ques_list
                vqadataset['ans_list'] = self.ans_list
                vqadataset['iid_to_frcn_feat_path'] = self.iid_to_frcn_feat_path
                vqadataset['qid_to_ques'] = self.qid_to_ques
                vqadataset['token_to_ix'] = self.token_to_ix
                vqadataset['pretrained_emb'] = self.pretrained_emb
                vqadataset['token_size'] = self.token_size
                vqadataset['ans_to_ix'] = self.ans_to_ix
                vqadataset['ix_to_ans'] = self.ix_to_ans
                vqadataset['ans_stat'] = self.ans_size

                # dump into cache
                pickle.dump(vqadataset, open('cache/vqa_' + __C.RUN_MODE + '.pkl', 'wb'), protocol=pickle.HIGHEST_PROTOCOL)

        # ------------------------
        # ------- Toy Dataset ----
        # ------------------------

        print('checking for toy dataset which is {}'.format(__C.TOY_DATASET))

        if __C.TOY_DATASET:
            if __C.RUN_MODE in ['train']:
                self.ques_list = self.ques_list[:__C.TOY_TRAIN_SAMPLES]
                self.ans_list = self.ans_list[:__C.TOY_TRAIN_SAMPLES]
            elif (__C.RUN_MODE in ['val']):
                self.ques_list = self.ques_list[:__C.TOY_VAL_SAMPLES]
                self.ans_list = self.ans_list[:__C.TOY_VAL_SAMPLES]
        
        self.ques_ids = [ques['question_id'] for ques in self.ques_list]
    ...
    def load_ques_ans(self, idx):
        ...
        # Process augmented answer
        qu = ques['question']
        qu_proc = self.proc_extras(qu)

        ty = ans['question_type']
        ty_proc = self.proc_extras(ty)

        an = ans['multiple_choice_answer']
        an_proc = self.proc_extras(an)
        
        aug_ans = qu.lower().replace(ty, an)
        aug_ans_proc = self.proc_extras(aug_ans)
        # creating a dict with question key since proc_ques requires that
        aug_ans_dict = {}
        aug_ans_dict['question'] = aug_ans 
        aug_ans_ix_iter = self.proc_ques(aug_ans_dict, self.token_to_ix, max_token=14)
        ...
        ret_dict = {
            ...
            'ans_iter': ans_iter,
            'iid': iid,
            'question': ('_').join(qu_proc),
            'augmented_answer': ('_').join(aug_ans_proc),
            'answer': ('_').join(an_proc),
            'question_type': ('_'.join(ty_proc)),
            'ques_id': ques_id,
            'aug_ans_ix_iter': aug_ans_ix_iter
        }
