...
class DataSet(BaseDataSet):
    def __init__(self, __C):
        ...
        if self.__C.USE_NEW_QUESTION is False:
        ...
        if self.__C.USE_NEW_QUESTION is "true" and self.__C.RUN_MODE is "test":

            print("Loading image features of image_id: 1")
            # Loading all image paths
            frcn_feat_path_list = \
                glob.glob(__C.FEATS_PATH[__C.DATASET]['test'] + '/COCO_test2015_000000000001.jpg.npz')

            print("Loading the specified question")
            # Loading question word list
            stat_ques_list = [
                {'image_id': 1, 'question': self.__C.NEW_QUESTION 'question_id': 100000001},
            ]
