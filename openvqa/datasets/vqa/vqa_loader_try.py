...
class DataSet(BaseDataSet):
    def __init__(self, __C):
        ...
        sys.exit("###########\tManual exit by Apoorve in /openvqa/datasets/vqa/vqa_loader_try.py\t############")
    ...
    def load_ques_ans(self, idx):
        if self.__C.RUN_MODE in ['train']:
            ...
            # Process question
            # List of size 14 with words ids, and word id of 'UNK' for unknown words, word id of 'PAD' for padded words
            ques_ix_iter = self.proc_ques(ques, self.token_to_ix, max_token=14)

            # Process answer
            # returns a list of size ans_to_ix where each position contains a score [0-1] that the prediction for that word will secure
            ans_scores = self.proc_ans(ans, self.ans_to_ix)
            
            #Edits
            # List of size 4 with words ids, and word id of 'UNK' for unknown words, word id of 'PAD' for padded words
            ans_ix_iter = self.proc_ans_tokens(ans, self.token_to_ix_ans, max_token = 4)
            #End of Edits

            return ques_ix_iter, ans_ix_iter, ans_scores, iid
            ...
            #TODO
            # will have to check, at the time of eval how is processing done
    ...
    def proc_ans(self, ans, ans_to_ix):
        ...
        # Iterate over all answers objects
