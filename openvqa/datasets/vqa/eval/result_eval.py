...
from visualization.wandb_utils import log_metrics
import json, pickle
import numpy as np
...
def eval(__C, dataset, ans_ix_list, pred_list, result_eval_file, ensemble_file, log_file, ques_ids = None, valid=False, **kwargs):
    ...
    if valid or kwargs["train"]:
        # create vqa object and vqaRes object
        if valid:
            ques_file_path = __C.RAW_PATH[__C.DATASET][__C.SPLIT['val']]
            ans_file_path = __C.RAW_PATH[__C.DATASET][__C.SPLIT['val'] + '-anno']
        elif kwargs["train"]:
            ques_file_path = __C.RAW_PATH[__C.DATASET][__C.SPLIT['train']]
            ans_file_path = __C.RAW_PATH[__C.DATASET][__C.SPLIT['train'] + '-anno']
...
#            send_message_to_slack(slack_message)
        ...
        acc_dict["Overall Accuracy"] = vqaEval.accuracy['overall']
        for ansType in vqaEval.accuracy['perAnswerType']:
        ...
        if valid:
            log_metrics(acc_dict, "val", kwargs["epoch"])
        elif kwargs["train"]:
            log_metrics(acc_dict, "train", kwargs["epoch"])
        logfile.write("\n\n")
        logfile.close()
