...
def test_engine(__C, dataset, state_dict=None, validation=False, epoch = 0):
    ...
    for step, (
        ...
        pred_np_squeezed = pred_np.squeeze()
        res = sorted(range(len(pred_np_squeezed)), key = lambda sub: pred_np_squeezed[sub])[-5:]
    ...
    EvalLoader(__C).eval(dataset, pred_np_squeezed, res, ans_ix_list, pred_list, result_eval_file, ensemble_file, log_file, validation)
